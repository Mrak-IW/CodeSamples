...
namespace Matrix
{
	...
	public class TMatrix
	{
		...
		/// <summary>
		/// Преобразует матрицу в одномерный массив short[], разворачивая её построчно.
		/// </summary>
		/// <returns></returns>
		public short[] ToShortPlainArray()
		{
			short[] res = new short[this.Heigth * this.Width];
			int i;

			for (i = 0; i < this.Heigth * this.Width; i++)
			{
				res[i] = Convert.ToInt16(this[i / this.Width, i % this.Width]);
			}
